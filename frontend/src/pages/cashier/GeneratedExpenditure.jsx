...
const downloadExpenditureCsv = (groupedByDate, title) => {
  ...
  groupedByDate.forEach(({ date, rows, subtotal }) => {
    rows.forEach((r) => {
      lines.push(
        [srNo, date, headCode(r), r.voucherNo, r.grossAmountNum.toFixed(2)]
          .map(csvField)
          .join(","),
      );
      srNo++;
...
// ── Modal: rows grouped by date — Sr No, Date, Head Code, Voucher No, Gross
...
const ExpenditureListModal = ({ title, rows, onClose }) => {
  const groupedByDate = useMemo(() => {
    ...
    return Array.from(byDate.entries()).map(([date, groupRows]) => ({
      date,
      rows: groupRows,
      subtotal: groupRows.reduce((sum, r) => sum + (r.grossAmountNum || 0), 0),
    }));
  }, [rows]);
